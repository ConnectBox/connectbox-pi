...
def process_dir(top_dir, dest_dir, opts):
    ...
    path_top_dir = os.path.join(top_dir, "")
    index_file = ""
...
<header>
    ...
    print ("wrote inital html to file")

    # sort dirs first
    sorted_entries = sorted(path_top_dir.glob(glob_patt), key=lambda p: (p.is_file(), p.name))

    print ("sorted entries are: ",sorted_entries)

    entry: Path
    for entry in sorted_entries:

        print ("working on entry ", entry.name.lower())
        ...
        if ((entry.is_dir()) and ('recursive' in opts)):
            new_dir = Path.resolve(entry)
            for path2, dirs2, files2 in os.walk(new_dir):
                if ((len(files2)==0) and (len(dirs2) != 0)):
                    if ('recursive' in opts): opts2 = ""
                    else: opts2 = opts
                    print ("new_dir is: ",new_dir)
                    print ("were going to recurse with opts: ", opts2)
                    process_dir(new_dir, new_dir,  opts2)
