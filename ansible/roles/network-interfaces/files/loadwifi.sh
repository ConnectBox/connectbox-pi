...
then
  echo "Loading 88x2bu"
  insmod /usr/lib/modules/$( uname -r)/kernel/drivers/net/wireless/88x2bu		#We found nothing so try to load this versions 88x2bu
  modprobe 88x2bu
  depmod -A
else
...
then
  echo "Loading 8852au"
  insmod /usr/lib/modules/$( uname -r)/kernel/drivers/net/wireless/8852au		#We found nothing so try to load this versions 88x2bu
  modprobe 8852au
  depmod -A
else
