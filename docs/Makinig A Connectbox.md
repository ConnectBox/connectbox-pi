...
###Section 3:	- Building a NEO or RPi image in a Mac ###environment Section 4:	- Creating a Raspberry Pi based ###build environment Section 5:	- Building a NEO or RPi image in an RPi environment Section 6:	- Shrinking the image
...
1. Prepare the build environment (?One o? steps?).
...
~# some_command
(note the di?erence between the ?~$? and ?~#?.)
...
- In the left pane of the application window, check to see if the virtual machine is running. If it shows ?Running?, right click on the blue box with the virtual machine name and select ?Close -> Power O??.
- In the next dialog, click ?Power O??.
...
- Now insert a USB memory stick and click that same icon again. You should see
your USB stick listed as "Generic USB Mass Storage Device [0500]". (Perhaps greyed out because the Apple system has mounted it and so it isn?t available to the VB. That is OK. If you go to Finder and Eject the stick but don?t physically
unplug the USB stick, then go back and again click the small USB icon, you will see it listed among the devices and NOT greyed out.)
...
? Where there are di?erences between building a NEO image and an RPi image, we will preface the step number with "N" (NEO) or "R" (RPi). Where there is no di?erence, the step number will be shown without a prefaced letter.

Step N3.1: Use an internet browser to go to https://github.com/ConnectBox/armbian- build (the armbian build repo). Near the top you will see (on the line just above ?Clone or download?) something like, ?6220 commits 9 branches 0 packages 15 releases
...
Step 3.2: Use balenaEtcher to burn the base image (from Step 3.1) to a uSD card. Step R3.3: This step only required if we are making an RPi image.
...
Step 3.10: Connect a wifi hub antenna to a USB connector on the target machine. (The build will fail if it doesn?t find a USB wifi dongle.) Lots of o?erings on Amazon. Here is one possibility:
...
Typical script queries and your responses: GitHub token [..]: <CR>
Tag for release [v20200508]: <CR>
...
Step 4.2: **Optional: **Change the name of the hostname from raspberrypi to RPi4 by accessing the dialog Preferrences ? RaspberryPi Configuration. (Pulldown the raspberry icon at the top left of the screen.) Change the Hostname to RPi4 (or your preferrence) and then click OK. Then do a sudo edit of /etc/hosts to change "localhost" to the new name. Reboot for the changes to take e?ect.
...
Step 5.10: Connect a wifi hub antenna to a USB connector on the target machine. (The build will fail if it doesn?t find a USB wifi dongle.) Lots of o?erings on Amazon. Here is one possibility:
https://www.amazon.com/Adapter-600Mbps-802-11ac-Wireless-Compatible/dp/ B086WDF6VF/ref=sr_1_15? dchild=1&keywords=usb+wifi+dongle&qid=1589402526&sr=8-15
...
The following are instructions for shrinking the image on a uSD card. The process is very similar for both Mac and RPi development machines. Where di?erences exist, we will preface the step number with "M" (Mac) or "R" (Rpi).
...
3. The build process tags several repos (see the	in https://
...
commands listed in the repository.

of the

3. Checkout the connectbox-tools repo, cd into the checkout and run

directory of the


**Per-build**
This assumes you're using the automated machine setup above. If you're using the

manual method, you can probably replace
...
repo that you've checked out on the machine.
Update the CHANGELOG.md file in the connectbox-pi repository to list any changes since the last build
Prepare a your target device with a base operating system image. On a NEO, use the most recent release from the https://github.com/ConnectBox/armbian-

build repository. For an RPi, try the most recent release of Raspbian lite, but know that ymmv because their image changes.
3. Login to the virtual machine that you've setup in the Release Machine Setup
section. You can do that by running	in the
directory to login to the virtual machine.
4. Make sure that you can perform a passwordless login as root on the target
...
easy if you have keys already available, and automates the 4 steps below, assuming you're building a NEO image - it's di?erent for an RPi)
...
(pwd: 1234)
Set root password to connectbox per the prompts Ctrl^c and do not create any extra accounts
Put your ssh public key on the device so you don?t need to run ansible with

on the release machine and follow the
instructions.
