...
if [ "$TRAVIS_BRANCH" = "master" ]; then
  ...
  ansible-playbook -i ci-inventory -e shutdown_in_image_prep=False -e do_image_preparation=True ../ansible/site.yml || exit 1;
else
  # Do essential steps of a deployment to keep things fast
  ansible-playbook -i ci-inventory -e shutdown_in_image_prep=False -e do_image_preparation=True --skip-tags=full-build-only ../ansible/site.yml || exit 1;
fi
