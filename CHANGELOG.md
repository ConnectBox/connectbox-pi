...
* Fixed: Memory leak in waypoint-hat-service that would cause lockup of devices running a Q3Y2018 HAT after a few hours.
...
* Fixed: Allow sshd to be enabled on Raspberry Pi devices by the usual .waypoint/enable-ssh method via USB storage
* Added: "Return to main interface" button in the admin area [Issue #274](https://github.com/waypoint/waypoint-pi/issues/274)

# 20181016

* Changed: Upgrade to Ansible 2.6.5.
* Changed: Run image preparation logic in CI
* Removed: Many OS packages that are not required for waypoint operation but took disk space
...
* Changed: Extract captive portal into waypoint/simple-offline-captive-portal
...
* Fixed: Android 7 devices only showed captive portal instructions momentarily [Issue #251](https://github.com/kirkdwilson/MV-WayPoint/waypoint-pi/issues/251)
...
* Fixed: Android 8 devices with active celluar connections were unable to remain connected to the captive portal [Issue #250](https://github.com/kirkdwilson/MV-WayPoint/connectbox-pi/issues/250)

# 20180708

* Fixed: Admin system actions report "not initiated" despite performing action [Issue #247](https://github.com/kirkdwilson/MV-WayPoint/connectbox-pi/issues/247)
...
* Changed: Images no longer have ssh enabled by default. To enable ssh, create .waypoint/enable-ssh on your USB storage, insert the USB storage and boot the device. Please change the root password immediately after booting
...
* Added: Complete disabling of automatic OS updates (allowing ansible-playbook runs to complete without failing on dpkg locks) [Issue #214](https://github.com/kirkdwilson/MV-WayPoinit/connectbox-pi/issues/214)
...
* Fixed: Folder icon rendering issues [Issue #233](https://github.com/kirkdwilson/MV-WayPoint/connectbox-pi/issues/233)
* Fixed: NEO ethernet MAC changes at power cycle [Issue #237](https://github.com/kirkdwilson/MV-WayPoint/connectbox-pi/issues/237)
* Added: Packages and system overlays required for future OLED integration
* Added: waypoint build number under /etc/waypoint-release (not exposed in UI yet)
...
* Fixed: Corrupted /boot/armbianEnv.txt on unclean shutdown [Issue #220](https://github.com/kirkdwilson/MV-WayPoint/connectbox-pi/issues/220)
...
* Removed: Automatic redirection to waypoint page when attempting to load other sites
* Fixed: Constrain Wifi channel selection to valid values in the admin UI
* Added: Initial implementation of chat API and UI
* Added: Install and activate battery level shutdown for NanoPi NEO devices with a waypoint Hat
...
* Fixed: Channel 11-14 unavailable regardless of regulatory domain [Issue #168](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/168)
* Changed: Do not allow 40Mhz 2.4Ghz channels [PR #170](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/pull/170)
* Fixed: Image does not expose all available space on the microsd card [Issue #163](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/163)
* Fixed: File listings do not have a cache policy [PR #172](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/pull/172)
* Changed: Disable automatic apt upgrades [PR #173](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/pull/173)

# 20170901

* Fixed: Ethernet MAC address changes on each reboot: [Issue #124](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/124)
* Fixed: Image does not expose all available space on the microsd card [Issue #163](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/163)
* Added: Attempt to set the WiFi Regulatory domain automatically based on the regulatory domain of surrounding devices [PR #166](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/pull/166)

# 20170827

* Fixed: Admin Interface - Unable to use input boxes on small screen ([Issue #158](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/158))
* Fixed: Admin interface - error dialog button off screen ([Issue #157](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/157))
* Fixed: Setting Wifi channel, Hostname or SSID through admin interface while on wifi reports an error ([Issue #156](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/156))
* Added: Allocate DHCP addresses with a 24h lease instead of 2min and from a /16 address space instead of a /24 ([PR #164](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/pull/164))

# 20170820

* Removed: Images based on the legacy images are no longer produced based on results of [legacy vs mainline testing](https://github.com/kirkdwilson/MV-WayPoint//wifi-test-framework/blob/master/reports/legacy_and_mainline.md).
* Removed: The unprivileged `box` account is no longer present on new images.
* Added: Improved WiFi performance by activating high-throughput capabilities for the RT5372 device, based on the results of [ht_capab testing](https://github.com/kirkdwilson/MV-WayPoint//wifi-test-framework/blob/master/reports/htcapab.md)
* Changed: The default password for root has changed on new images. See the [Release Notes](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/wiki/Release-Notes:-25-unit-pilot) for the new default.
...
* Fixed: Standard Operating System folders on USB storage are not hidden in Icon-only interface: [Issue #118](https://github.com/kirkdwilson/MV-WayPoint//connectbox-pi/issues/118)
...
* Fixed: Linux desktops could not connect to the web interface using the default WayPoint hostname
